...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT patient_id, patient_username ,patient_password
				  FROM patient
                                  WHERE patient_email = ? LIMIT 1")) {
...
function login_d($email, $password, $mysqli) {
    // Using prepared statements means that SQL injection is not possible. 
    if ($stmt = $mysqli->prepare("SELECT doctor_id, doctor_username ,doctor_password
				  FROM doctor
                                  WHERE doctor_email = ? LIMIT 1")) {
        $stmt->bind_param('s', $email);  // Bind "$email" to parameter.
        $stmt->execute();    // Execute the prepared query.
        $stmt->store_result();
